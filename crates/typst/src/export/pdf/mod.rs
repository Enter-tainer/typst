...
fn write_catalog(ctx: &mut PdfContext, timestamp: Option<Datetime>) {
    ...
    if let Some(timestamp) = timestamp {
        if let Some(year) = timestamp.year().and_then(|y| u16::try_from(y).ok()) {
            ...
            if let Some(m) = timestamp.month() {
                date = date.month(m);
            }

            if let Some(d) = timestamp.day() {
                date = date.day(d);
            }

            if let Some(h) = timestamp.hour() {
                date = date.hour(h);
            }

            if let Some(m) = timestamp.minute() {
                date = date.minute(m);
            }

            if let Some(s) = timestamp.second() {
                date = date.second(s);
            }
